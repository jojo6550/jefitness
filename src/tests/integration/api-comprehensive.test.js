...
describe('Comprehensive API Integration Tests', () => {
  ...
  describe('Authentication Flow', () => {
    test('[1] Signup - Create new user account', skip(async () => {
    ...
    test('[2] Login - Authenticate with email and password', skip(async () => {
      if (!testToken) {
    ...
    test('[3] Get Current User - Verify authenticated session', skip(async () => {
      if (!testToken) return;
    ...
    test('[4] Login Failure - Reject invalid credentials', skip(async () => {
      const res = await client.post('/auth/login', {
        email: testEmail,
  ...
  describe('User Profile Management', () => {
    test('[5] Get Profile - Retrieve user profile', skip(async () => {
      if (!testToken) return;
      const res = await client.get('/users/profile');
      expect([200, 401, 404]).toContain(res.status);
    }), 30000);

    test('[6] Update Profile - Modify user information', skip(async () => {
      if (!testToken) return;
    ...
    test('[7] Add Measurements - Record body measurements', skip(async () => {
      if (!testToken) return;
    ...
    test('[8] Get Measurements - Retrieve measurement history', skip(async () => {
      if (!testToken) return;
    ...
    test('[9] Privacy Settings - Update user privacy preferences', skip(async () => {
      if (!testToken) return;
  ...
  describe('Workout Tracking', () => {
    ...
    test('[10] Create Workout - Log exercise session', skip(async () => {
      if (!testToken) return;
    ...
    test('[11] List Workouts - Retrieve workout history with pagination', skip(async () => {
      if (!testToken) return;
    ...
    test('[12] Get Specific Workout - Retrieve details of one workout', skip(async () => {
      if (!testToken || !workoutId) return;
      const res = await client.get(`/workouts/${workoutId}`);
      expect([200, 401, 404]).toContain(res.status);
    }), 30000);

    test('[13] Update Workout - Modify exercise details', skip(async () => {
      if (!testToken || !workoutId) return;
    ...
    test('[14] Delete Workout - Remove workout log', skip(async () => {
      if (!testToken || !workoutId) return;
  ...
  describe('Nutrition Tracking', () => {
    ...
    test('[15] Create Meal Log - Record food intake', skip(async () => {
      if (!testToken) return;
      const res = await client.post('/nutrition/logs', {
        mealType: 'breakfast',
        date: new Date().toISOString(),
        foods: [
          { name: 'Chicken Breast', quantity: 150, unit: 'g', calories: 250, protein: 40, carbs: 0, fat: 5 },
    ...
    test('[16] List Meal Logs - Retrieve nutrition history', skip(async () => {
      if (!testToken) return;
    ...
    test('[17] Daily Macro Summary - Get nutritional summary for date', skip(async () => {
      if (!testToken) return;
    ...
    test('[18] Delete Meal Log - Remove food record', skip(async () => {
  ...
  describe('Health Data Logging', () => {
    ...
    test('[19] Create Health Log - Record health metrics', skip(async () => {
      if (!testToken) return;
    ...
    test('[20] List Health Logs - Retrieve health data history', skip(async () => {
      if (!testToken) return;
    ...
    test('[21] Delete Health Log - Remove health record', skip(async () => {
  ...
  describe('Public Endpoints', () => {
    test('[22] Get Programs - List available fitness programs', skip(async () => {
    ...
    test('[23] Get Plans - List available subscription plans', skip(async () => {
    ...
    test('[24] Server Health Check - Verify server is running', skip(async () => {
  ...
  describe('Account Operations', () => {
    test('[25] Request Password Reset - Initiate reset flow', skip(async () => {
    ...
    test('[26] Request Email Verification - Send verification email', skip(async () => {
      if (!testToken) return;
      const res = await client.post('/auth/verify-email-request', {});
      expect([200, 400, 401, 429]).toContain(res.status);
    }), 30000);

    test('[27] Logout - End authenticated session', skip(async () => {
      if (!testToken) return;
  ...
  describe('Error Handling', () => {
    test('[28] Unauthorized Access - Reject unauthenticated protected routes', skip(async () => {
      const res = await client.get('/users/profile');
      expect([401, 403]).toContain(res.status);
    }), 30000);

    test('[29] Invalid Data - Reject malformed requests', skip(async () => {
      const res = await client.post('/auth/signup', {
        email: 'not-an-email',
        password: 'short',
      });
    ...
    test('[30] Not Found - Handle missing resources', skip(async () => {
      if (!testToken) return;
