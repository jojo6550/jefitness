...
async function main() {
  try {
    ...
    switch (cmd) {
      ...
      case 'list':
        await connectDB();
        const plans = await StripePlan.find({ active: true }).sort({ unitAmount: 1 }).lean();
      ...
      case 'remove':
        if (!commands[1]) return console.error('❌ node cli-commands.js remove <priceId>'), process.exit(1);
        await connectDB();
        const res = await StripePlan.deleteOne({ stripePriceId: commands[1] });
        console.log(res.deletedCount ? `🗑️ Removed ${commands[1]}` : `ℹ️ Not found: ${commands[1]}`);
        break;

      case 'add-lookup':
        if (commands.length !== 3) return console.error('❌ node cli-commands.js add-lookup <key> <priceId>'), process.exit(1);
        await connectDB();
        const upd = await StripePlan.updateOne({ stripePriceId: commands[2] }, { $set: { lookupKey: commands[1] } });
        console.log(upd.matchedCount ? `✅ lookupKey "${commands[1]}" → ${commands[2]}` : `ℹ️ Not found: ${commands[2]}`);
        break;
